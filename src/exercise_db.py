...
class ExerciseDatabase:
        ...
        if os.path.exists(self.db_path):
            try:
                with open(self.db_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    for exercise_data in data.values():
                        exercise = Exercise(**exercise_data)
                        self.exercises[exercise.id] = exercise
            except Exception as e:
                print(f"데이터베이스 로드 중 오류 발생: {str(e)}")
    ...
    def add_exercise(self, exercise: Exercise) -> None:
        ...
        if exercise.id in self.exercises:
            raise ValueError(f"이미 존재하는 운동 ID입니다: {exercise.id}")
    ...
    def update_exercise(self, exercise_id: str, updated_data: Dict) -> None:
        """
        운동 정보 업데이트
        ...
        if exercise_id not in self.exercises:
            raise ValueError(f"존재하지 않는 운동 ID입니다: {exercise_id}")
    ...
    def get_exercise_statistics(self) -> Dict:
        ...
        for exercise in self.exercises.values():
            # 난이도 분포
            stats["difficulty_distribution"][exercise.difficulty_level] += 1
